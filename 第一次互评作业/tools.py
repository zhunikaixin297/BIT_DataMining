...
def column_loader(parquet_dir, columns=None, chunksize=5e6):
    """分块加载器"""
...
def process_expansion(df, required_fields=None):
    ...
    try:
        ...
        merged = pd.concat([
            ...
            df_purchase[[col for col in ['purchase_avg_price', 'purchase_categories']
                        if (required_fields is None) or (col in required_fields)]]
        ], axis=1)
...
def preprocess(parquet_dir, output_dir, columns_to_keep=None):
    ...
    if columns_to_keep is None:
        columns_to_keep = [
            'id', 'last_login', 'user_name', 'fullname', 'age',
            'income', 'gender', 'country', 'purchase_avg_price',
            'is_active', 'purchase_categories'
        ]
    ...
    for chunk_idx, chunk in enumerate(chunk_gen):
        ...
        # 三字段复合去重
        hash_str = (
                chunk['id'].astype(str) + "_" +
                chunk['last_login'].astype(str) + "_" +
                chunk['user_name'].astype(str)
        )
        ...
        # 关键字段缺失值处理
        key_cols = ['id', 'last_login', 'user_name'] + NUMERIC_COLS
...
def get_dataset_summary(parquet_dir):
    ...
    # 预定义字段分类
    num_fields = ['id', 'age', 'income',  'purchase_avg_price']
    cat_fields = ['gender', 'country', 'is_active', 'purchase_categories']
